...
const STORAGE_KEY = "song-buckets-v1"; // localStorage key
        ...
        {["Lyrics", "Production", "Vibe"].map((b) => (
          <div key={b} className="bin">
            ...
            <div style={{ marginTop: 6 }}>
              {inB(b) ? (
                <span className="tag">{song.title}</span>
              ) : (
                <span className="small">Click “Add here” to include</span>
              )}
            </div>
            <div style={{ marginTop: 8 }}>
              <button className="tag" onClick={() => toggle(b)}>
                {inB(b) ? "Remove" : "Add here"}
              </button>
            </div>
          </div>
        ))}
